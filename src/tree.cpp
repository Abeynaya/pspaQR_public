...
/* Solve with W -- for LSQR */
void Tree::solveW(VectorXd b, VectorXd& x) const{
    assert(b.size()== ncols);
    VectorXd bpad = VectorXd::Zero(nrows);
    bpad.head(ncols) = b;

    // Permute the rhs according to this->rperm
    bpad = this->rperm.asPermutation().transpose()*bpad;

    // Set solution
    for (auto cluster: bottom_original()){
        cluster->set_vector(bpad);
    }
...
/* Solve with W^T -- for lsqr */
void Tree::solveWt(VectorXd b, VectorXd& x) const{
    assert(b.size() == this->ncols);
    assert(x.size() == this->ncols);
    ...
    VectorXd xpad = VectorXd::Zero(nrows);
    // Extract solution
    for(auto cluster : bottom_original()) {
        cluster->textract_vector(xpad);
    }

    // Permute back
    xpad = this->rperm.asPermutation() * xpad;

    // x is only the top ncols
    x = xpad.head(ncols);
}
